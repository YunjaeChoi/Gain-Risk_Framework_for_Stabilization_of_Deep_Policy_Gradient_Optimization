...
from utils.logx import EpochLogger, CSVLogger
import model
from lr_scheduler import LinearLR
from replay_buffer import AdvantageBuffer as Buffer

def ppo(env_fn, actor_critic=model.mlp_actor_critic, seed=0,
    ...
    pi_loss = -tf.reduce_mean(tf.minimum(ratio * adv_ph, adv_th))
    ...
    def update(epoch):
        ...
        adv, ae, obs, adn, amd, asd = sess.run([adv_ph, adv_eta, x_ph, a_dn_ph, act_mu_dn, act_std_dn], feed_dict=pi_inputs)
        opt_results_logger.store(Adv=np.copy(adv), Obs=np.copy(obs), Adn=np.copy(adn),
            AdvEta=np.copy(ae), Amudn=np.copy(amd), Astddn=np.copy(asd))
