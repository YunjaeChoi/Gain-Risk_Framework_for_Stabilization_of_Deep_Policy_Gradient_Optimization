...
class Logger:
    ...
    def save_model(self, itr=None):
        if hasattr(self, 'tf_saver_elements'):
            self._tf_simple_save(itr)
            print(colorize('Model saved.', color='cyan', bold=True))
...
class CSVLogger(Logger):
    ...
    def save_csv(self, file_name):
        concat_data = {k:np.concatenate(v, axis=0) for k,v in self.log_dict.items()}
        #2d data only(1d -> 2d)
        concat_data = {k:v if len(v.shape)==2 else np.expand_dims(v, axis=-1) for k,v in concat_data.items()}
        df_list = [
            pd.DataFrame(v, columns=[k + str(i) for i in range(v.shape[1])])
            for k,v in concat_data.items()
        ]
        data = pd.concat(df_list, axis=1)
        output_fpath = osp.join(self.output_dir, file_name)
        data.to_csv(output_fpath, index=False)
...
class NPLogger(Logger):
    ...
    def save_array(self, file_name):
        for k in self.log_dict.keys():
            array = np.stack(self.log_dict[k], axis=0)
            output_fpath = osp.join(self.output_dir, file_name + k)
            np.save(output_fpath, array)
